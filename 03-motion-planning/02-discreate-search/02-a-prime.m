...
while true
    ...
    for k=1:nadjs
        ...
        if (...
            ...
            if (map(next_ind) ~= 3) && (map(next_ind) ~= 5) && (f(next_ind) > g(current) + 1 + H(next_ind))
                % update parent:
                parent(next_ind) = current;
                % update minimum path cost and heuristic total cost:
                g(next_ind) = g(current) + 1;
                f(next_ind) = g(next_ind) + H(next_ind);
                % update node state:
                map(next_row, next_col) = 4;
            end
        end
    end
    %*********************************************************************
